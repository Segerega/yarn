...
class JAS_Addons {
	...
	function __construct() {
		...
		load_plugin_textdomain( 'jsa', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
	...
	public static function shortcodes( $shortcodes = NULL ) {
		$shortcodes = 'service, portfolio, member, blog, products, product, google_maps, wc_categories, instagram, vertical_slide, vertical_slide_left, vertical_slide_right, vertical_slide_section, promotion, slider';
		return $shortcodes;
	}
	...
	public static function frontend_scripts() {
		...
		if ( is_singular() ) {
			...
			$message = function_exists( 'cs_get_option' ) ? cs_get_option( 'wc-single-shipping-return' ) : '';

			if ( has_shortcode( $post->post_content, 'jas_google_maps' ) || has_shortcode( $message, 'jas_google_maps' ) ) {
